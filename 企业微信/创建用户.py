...
def createuser(code,tel):
    ACCESS_TOKEN=get__token(appid,sceret)
    sql='''
select u.user_name,u.byname,d.dept_name,dd.vid from user u left join department d on u.dept_id=d.dept_id left join wx_dept dd on dd.name=d.dept_name'''
    ...
    try:
        list=[itme for itme in rs if itme[1]==code]
        dept_id=list[0][3]
        name=list[0][0]
    except Exception as e:
        print("异常请手工补充")
